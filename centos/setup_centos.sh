...
# CentOS/Redhat Server Configuration -- a basic script to automate some common configuration tasks
...
print_banner() {
    cat <<EOF
┌─────────────────────────────────────────────────────────┐
│             CentOS Server Configuration 0.1             │
└─────────────────────────────────────────────────────────┘
EOF
}
...
install_packages() {
    $sudo yum update
    $sudo yum install -y ${packages["${release}"]}
    return 0
}
